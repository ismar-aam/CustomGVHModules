...
const libraryId = argv.libraryId || "AAMCustomModule";
const namespace = argv.namespace || "geocortex/custom";
/** 
...
function bundleFiles() {
    ...
    modules.forEach(moduleName => {
        ...
        //let sourceMapsPath = glob.sync(`${sourcePath}/${moduleName}/**/*.js.map`);

        //console.log(`Source Maps Path ${sourceMapsPath}`);
        ...
        if (moduleName.toLowerCase().endsWith(".ts") && files.length === 0) {
            ...
            //console.log(`Source Maps Path For ts ${sourceMapsPath}`);
        }
        ...
        //if (moduleName.toLowerCase().endsWith("js.map") && files.length === 0) {
        //    sourceMapsPaths = glob.sync(`${sourcePath}/${moduleName}`);
        //    console.log(`Source Maps Path Found ${sourceMapsPath}`);
        //}
...
gulp.task("ts-build", () => {
    ...
    const streams = [tsResult.js
        //.pipe(sourcemaps.init())
        //.pipe(sourcemaps.write())
        .pipe(processTypescriptOutput())
        ];
